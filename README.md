...
2. `./server/ssr.js` compiles isomorphic React.js code into HTML (SSR mode), attaches `main.bundle.js` to the HTML and sends it on the browser's HTTP GET request.
...
- `/client` — browser code
- `/server` — server code
- `/shared` — isomorphic react code which can be run both — in CSR or SSR.
